...
class Teenager: Babysitter {
  ...
  func playCandyland(_ numberOfTimes: Int) {
    print("Playing Candyland \(numberOfTimes) times.")
  }
...
let kei: Babysitter = Teenager(age: 16, responsible: true, patience: .high)
kei.playCandyland(10)
kei.read("The Great Gatsby", firstLine: "...", asleep: false)
...
class Person {
  ...
  convenience init() {
    self.init(name: "Hello", age: 10)
  }
  
  convenience init(age: Int) {
    self.init(name: "Hello", age: age)
  }
}
let p = Person()
let p1 = Person(age: 10)
let p2 = Person(name: "Dan", age: 12)
...
let _ = UIColor(redValue: 197, greenValue: 255, blueValue: 0)
