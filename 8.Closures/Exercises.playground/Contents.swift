...
let orderedSurnames = surnames.sorted{ $1 < $0 }

// 2
let battingAverages = [0.302, 0.556, 0.280, 0.500, 0.281, 0.285]
let sortedAverages = battingAverages.sorted{ $1 < $0 }
...
let filtered = numbersAsStrings.filter {
    guard let num = Int($0) else { return false}
    return num % 12 == 0 ? true : false
}
//let filtered = numbersAsStrings.filter { (elem) -> Bool in
//    if let num = Int(elem), num % 21 == 0 {
//        return true
//    }
//    return false
//}
...
let smallerThan20 = particleSizesInMicrons.filter { $0 < 20 }
...
let canadianDoller = pricesInRupees.map { Double($0) * 0.019 }
//: __Problem 6__
//:
...
let seconds = oldTimes
    .map (timeIntervalFromString)
    .map{ $0 + 13 }
    .map(timeStringFromInterval)
